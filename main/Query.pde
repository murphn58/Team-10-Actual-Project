...
class Query{
  ...
  void sortByLateness()                  // sorts the attached returnTable by how "late" the arrival time is, jank implementation of difference between day change vs early
  {
    ...
    for(TableRow row: tempTable.rows()){
      ...
      }else    // column 14 = actual , column 13 = planned 
      {
        ...
        {
          int actualArrivalTime = Integer.parseInt(row.getString("ARR_TIME"));                                // reading the String time and making it into an int so it can be sorted using sort()
          int plannedArrivalTime = Integer.parseInt(row.getString("CRS_ARR_TIME"));
          
          int difference = actualArrivalTime - plannedArrivalTime;
          if(difference < 0)
          {
            if(actualArrivalTime < 500 && plannedArrivalTime >= 2200)    // if the difference changed days, as in if the planned arrival time is the day before the actual arrival time
            {
 ...
 void reset(){
   // resets the table attached to query to the original query
